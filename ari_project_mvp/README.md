...
5. 학생으로 로그인 → `/seats` — 배치도에서 좌석 선택 후 하단 **예약하기**
6. `/checkin/{id}` — **현장 침대/좌석에 붙은 QR을 카메라로 스캔**
   - 스캔이 되면 자동으로 체크인된다 (별도 입력 없음)
   - 카메라 권한을 거부했거나 카메라가 없으면 안내와 **다시 시도** 버튼이 나온다
7. 대시보드 → 퇴실하기

### 시나리오 2: 미인증 학생 예약 차단

1. `student2@ari.ac.kr` 로그인
2. 좌석 선택 후 예약하기 → "학생 인증이 필요합니다" 오류

### 시나리오 3: 잘못된 QR 체크인 차단

1. 예약 후 체크인 화면에서 **다른 좌석의 QR**을 비춘다
2. "예약하신 좌석의 QR이 아닙니다" 오류 → 2초 뒤 자동으로 다시 스캔
...
- **운영 방법:** 관리자 화면 > `/admin/qr-print`에서 좌석 QR을 A4로 인쇄(한 장에 12개)해 잘라서 해당 침대/좌석에 부착
- **체크인:** 학생 앱에서 `html5-qrcode`로 카메라 스캔만 지원한다. 토큰을 눈으로 보고 입력하는 경로는 없다
- **재발급:** QR 인쇄 화면에서 좌석별/전체 재발급 가능 (기존 스티커는 즉시 무효)
...
학우실은 3개 방, 총 28석이며 전부 2층 침대(사다리로 연결된 1층/2층 한 쌍)입니다.

| 방 (`location`) | `room_gender` | 좌석 | 침대조 (`bunk_group`) |
|---|---|---|---|
| 남학우실 일반방 | male | 10석 (A1-1 ~ A5-2) | A1 ~ A5 |
| 여학우실 일반방 | female | 12석 (A1-1 ~ A6-2) | A1 ~ A6 |
| 여학우실 굴방 | female | 6석 (B1 ~ B6) | B1(B1/B4), B2(B2/B5), B3(B3/B6) |

- `floor`: 1(아래 침대) / 2(위 침대). 굴방은 번호에 층이 없으므로 이 필드로 구분
- 좌석번호는 **같은 방 안에서만 유일**합니다 (남/여 일반방 모두 `A1-1` 사용)
- 이용가능/이용불가는 좌석 속성이 아니라 예약 현황으로 실시간 계산됩니다
- 장애인 배려 권장석(♿)은 표시만 하며 DB 필드가 아닌 `SeatsPage.jsx`의 `ACCESSIBLE_SEATS` 상수로 관리합니다
- 배치도 시안: [`docs/design/`](docs/design/)

> ⚠️ **좌석 위치는 `frontend/src/pages/student/SeatsPage.jsx`의 `ROOMS` 설정에 하드코딩되어 있습니다.**
> 좌석을 추가·삭제하거나 방/침대조를 바꾸면 DB(`backend/seed.py` 또는 관리자 화면)만이 아니라
> `ROOMS` 설정도 함께 수정해야 합니다. `ROOMS`에 없는 방이나 침대조의 좌석은 DB에 있어도 배치도에 표시되지 않습니다.
