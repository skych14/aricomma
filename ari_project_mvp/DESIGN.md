...
- 승인된 학생만 좌석 예약 가능
- 좌석 목록/현황 확인
- 관리자 좌석 CRUD + 좌석 QR 인쇄/재발급
- 예약 생성 (1인 1좌석, 중복 예약 방지)
- 좌석 부착 QR을 카메라로 스캔하는 체크인
- 퇴실 처리
...
[앱]    학생이 예약 후 현장에서 해당 좌석 QR을 카메라로 스캔 (html5-qrcode)
        스캔으로 얻은 토큰을 체크인 API에 그대로 보낸다
```
...
- `/seats` — 좌석 현황 그리드
- `/checkin/:reservationId` — 카메라로 좌석 QR을 스캔해 체크인

### 관리자
- `/admin` — 단일 대시보드 (4탭)
  1. 인증 요청
  2. 좌석 관리 (QR 인쇄 화면은 `/admin/qr-print`)
  3. 예약/이용 로그
  4. 감사 로그
